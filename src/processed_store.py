...
def build_index_from_output_dir(
    ...
    FINEP/CONIF/PRPPG-IFES/PROEX-IFES/CAPES/CNPq: chave = campo link do JSON.
    """
    data = _load_raw(path)
    ...
    proex_ifes_set = set(data["proex_ifes"])
    ...
    for filename in os.listdir(output_dir):
        ...
        elif orgao == "PROEX/IFES":
            link = (obj.get("link") or "").strip()
            if link:
                proex_ifes_set.add(link)
        elif orgao == "CAPES":
            link = (obj.get("link") or "").strip()
            if link:
                capes_set.add(link)
    ...
    data["proex_ifes"] = sorted(proex_ifes_set)
    ...
    with open(path, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    logger.info(
        "Built index from %s: fapes=%s, finep=%s, conif=%s, prppg_ifes=%s, proex_ifes=%s, capes=%s, cnpq=%s",
        output_dir,
        len(data["fapes"]),
        len(data["finep"]),
        len(data["conif"]),
